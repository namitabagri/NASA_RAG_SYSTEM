...
`embedding_pipeline.py` usable as a script entrypoint.
"""
...
class ChromaEmbeddingPipelineTextOnly:
    ...
    def __init__(self, 
        ...
        Flow:
            Initialize openai client --> Store config parameters --> Inititalize chromadb client -- > Embedding function --> Create collection
        """
       
        # Initialize OpenAI client
        api_key = os.getenv("OPENAI_API_KEY")
        self.openai_api_key = OpenAI(api_key=api_key)
        ...
        """Chromadb offers 2 ways for initialization:-
        1. Client: It stores the data in memory
        2. PersistentClient: It stores the data on disk, so even after the program has been run the data still stays."""
        self.chroma_client = chromadb.PersistentClient(path=chroma_persist_directory)
        
        # Create or get collection
        """
        When an embedding function is provided the chromadb converts the incoming input text into embeddings itself.
        Otherwise, the user has to handle the embeddings and then store them in the chromadb
        """
    ...
    def chunk_text(self, text: str, metadata: Dict[str, Any]) -> List[Tuple[str, Dict[str, Any]]]:
        ...
        while start < len(text):
            ...
            chunk_index += 1
         # Now update total_chunks in all metadata entries
    ...
    def get_embedding(self, text: str) -> List[float]:
        ...
        response = self.openai_client.embeddings_create(
            input= text,
    ...
    def add_documents_to_collection(self, documents, file_path, batch_size=50, update_mode='skip'):
        stats = {'added': 0, 'updated': 0, 'skipped': 0}

        # TODO: Handle 'replace' mode — delete all old chunks for this file first
    ...
    def process_all_text_data(self, base_path: str, update_mode: str = 'skip') -> Dict[str, int]:
        ...
        for file_path in files:
            try:
                ...
                stats['errors'] += 1  # TODO: Handle errors gracefully
